...
if 'NZBOP_SCRIPTDIR' in os.environ:
    if os.environ['NZBOP_VERSION'][0:5] < '11.0':
        print(f'[ERROR] NZBGet Version {os.environ["NZBOP_VERSION"]} is not supported. Please update NZBGet.')
        sys.exit(0)
    ...
    if 'NZBPP_TOTALSTATUS' in os.environ:
        if not os.environ['NZBPP_TOTALSTATUS'] == 'SUCCESS':
            print(f'[ERROR] Download failed with status {os.environ["NZBPP_STATUS"]}')
            status = 1
    ...
    if not os.path.isdir(os.environ['NZBPP_DIRECTORY']):
        print(f'[ERROR] Nothing to post-process: destination directory {os.environ["NZBPP_DIRECTORY"]} doesn\'t exist. Setting status "failed".')
        status = 1
...
def rename_script(dirname):
    ...
    for dir, dirs, files in os.walk(dirname):
        for file in files:
            if re.search(r'(rename\S*\.(sh|bat)$)', file, re.IGNORECASE) or file == 'What.sh':
    ...
    if rename_file:
        ...
        for line in rename_lines:
            ...
            if re.search('^(unrar)', line, re.IGNORECASE):
                ...
                print(f'[INFO] Extracting file {rename_file} with command {cmd}')
                ...
                print(f'[INFO] Checking for file {os.path.join(dirname, newfile)}')
                if os.path.isfile(os.path.join(dirname, newfile)):
                    print(f'[INFO] Reading lines from {os.path.join(dirname, newfile)}')
                    rename_lines2 = [
                        line2.strip() for line2 in open(
                            os.path.join(dirname, newfile),
                        )
                    ]
                    print(f'[INFO] Parsing {len(rename_lines2)} lines from {os.path.join(dirname, newfile)}')
                    for line2 in rename_lines2:
                        if re.search('^(mv|Move)', line2, re.IGNORECASE):
                            new_dir2 = rename_cmd(
                                shlex.split(line2)[1:], dirname,
                            )
                        if re.search('^(mkdir)', line2, re.IGNORECASE):
                            new_dir = os.path.join(
                                dirname, shlex.split(line2)[-1],
                            )
                    ...
                    print(f'[INFO] File {os.path.join(dirname, newfile)} not found')
            else:
                continue
    ...
    if new_dir:
        if CHMOD:
            print(f'[INFO] Changing file mode of {new_dir} to {oct(CHMOD)}')
        ...
        out_dir = os.path.join(
            os.path.split(
                dirname,
            )[0], os.path.split(new_dir)[1],
        )
        if not os.path.exists(out_dir):
            os.rename(dirname, out_dir)
            print(f'[NZB] DIRECTORY={out_dir}')


def rename_cmd(cmd, dirname):
    if len(cmd) == 2 and os.path.exists(os.path.join(dirname, cmd[0])):
        orig = os.path.join(
            dirname, cmd[0].replace(
                '\\', os.path.sep,
            ).replace('/', os.path.sep),
        )
        dest = os.path.join(
            dirname, cmd[1].replace(
                '\\', os.path.sep,
            ).replace('/', os.path.sep),
        )
        if os.path.isfile(dest):
            return
        print(f'[INFO] Renaming file {orig} to {dest}')
        try:
            ...
            print(f'[ERROR] Unable to rename file due to: {e}')
            sys.exit(NZBGET_POSTPROCESS_ERROR)
...
def extract_command(cmdin, dir):
    ...
    if platform.system() == 'Windows':
        ...
        if not os.path.exists(SEVENZIP):
            ...
            ext_7zip = [
                '.rar', '.zip', '.tar.gz', 'tgz',
                '.tar.bz2', '.tbz', '.tar.lzma', '.tlz', '.7z', '.xz',
            ]
